...
## Contacts
If there are $I(t)$ infected and $S(t)$ susceptibles, $p c S(t) I(t) / N = \beta S(t) I(t) /N$ is the number of individuals that are infected per unit time.

Therefore:

$\dfrac{d S(t)}{d t} = - \beta S(t) \dfrac{I(t)}{N}$

## Susceptible-Infected
Suppose that one cannot recover from the disease, and that the disease does not really change the mortality of individuals. The most fitting example is not the spread of an actual disease, but of rumors or other information.

We have only two equations. 

$\begin{cases}
\dfrac{d S(t)}{d t} = - \beta S(t) \dfrac{I(t)}{N}\\
\dfrac{d I(t)}{d t} = \beta S(t) \dfrac{I(t)}{N}
\end{cases}$

But because we keep the population fixed, $N = S(t) + I(t)$, we really have only one equation:

$\dfrac{d I(t)}{d t} = \beta S(t) \dfrac{I(t)}{N} = \beta (N - I(t)) \dfrac{I(t)}{N} = -\beta I(t) \left(1 - \dfrac{I(t)}{N}\right)$ 
...
Then, for any positive initial number of infected, $I(0)$, the whole population is infected in the end, as long as $\beta >0$. 
...
For example, influenza lasts about 5 days, meaning that $\gamma \approx 1/5$. We can therefore write:
...
$\begin{cases}
\dfrac{d S(t)}{d t} = -\beta S(t) \dfrac{I(t)}{N} + \gamma I(t)\\
\dfrac{d I(t)}{d t} = \beta S(t) \dfrac{I(t)}{N} - \gamma I(t)
\end{cases}$

Again, we really have only one equation, as $S(t) = N - I(t)$:

$\dfrac{d I(t)}{d t} = \beta \dfrac{I(t)}{N} (N - I(t)) - \gamma I(t)$

## Susceptible-Infected-Susceptible
With some rearranging, we obtain:

$\dfrac{d I(t)}{d t} = (\beta - \gamma) I(t) \left(1 - \dfrac{I(t)}{N -\frac{\gamma}{\beta}}\right)$
...
The model we've seen deals with epidemics, which is the spread of the disease over a short period of time (hence, we could consider the population to be fixed).

For endemic diseases (i.e., which are circulating and maintained within the population), we need to accound for births and deaths. For simplicity, say that the population is at equilibrium, such that the number of births is close to the number of deaths:
...
Where $\mu N = \mu S + \mu I + \mu R$. Using the same derviation done above, we find 
$\mathscr R_0 = \dfrac{\beta N}{\gamma + \mu}$

## Vaccination
Suppose that we vaccinate a proportion $v$ of all newborns. The equations become:

$\begin{cases}
\dfrac{d S(t)}{d t} = - \beta S(t) I(t) + \mu N (1-v) - \mu S(t) \\
\dfrac{d I(t)}{d t} = \beta S(t) I(t) - \gamma I(t) - \mu I(t) \\
\dfrac{d R(t)}{d t} =  \gamma I(t) - \mu R(t) + \mu N v
\end{cases}$

....

## Critical vaccination
We saw that the disease cannot spread when $(1-v) \mathscr R_0 < 1$

Then if we can vaccinate $v > 1 - \dfrac{1}{\mathscr R_0}$ of the newborns (or small children who haven't experienced the disease yet), we can eradicate the disease completely.
